...
-- Variable to store the window session file path
local window_session_file = nil

-- Function to save the current window layout
local function save_window_layout()
  window_session_file = vim.fn.tempname()
  vim.cmd('mksession! ' .. window_session_file)
  print("Saved window session to:", window_session_file)
end

-- Function to restore the saved window layout
local function restore_window_layout()
  if window_session_file and vim.fn.filereadable(window_session_file) == 1 then
    print("Restoring window session from:", window_session_file)
    vim.cmd('silent! source ' .. window_session_file)
    vim.fn.delete(window_session_file)
    window_session_file = nil
  else
    print("No window session to restore")
  end
end

-- Map <C-w>o to save the layout and make the current window the only one
vim.keymap.set('n', '<C-w>o', function()
  save_window_layout()
  vim.cmd('only')
end, { noremap = true, silent = true })

-- Map <C-w>u to restore the previous window layout
vim.keymap.set('n', '<C-w>u', restore_window_layout, { noremap = true, silent = true })
